		...
		<tr>
			<td colspan="6">
				<form id="searchForm" action='<c:url value='/prod/prodList.do'/>'>
					<input type="text" name="page" /> <input type="text"
						name="prodLgu" value="${condition.prodLgu}" /> <input type="text"
						name="prodBuyer" value="${condition.prodBuyer}" /> <input
						type="text" name="prodName" value="${condition.prodName}" />
				</form>
				<div data-pg-role="searchUI" data-pg-target="#searchForm">
					<select name="prodLgu" data-pg-init-value="${condition.prodLgu}">
						...
						<option value class='default'>제조사선택</option>
						<c:forEach items="${buyerList}" var="buyer">
							<option value="${buyer.buyerId }" class="${buyer.buyerLgu}">${buyer.buyerName }</option>
						</c:forEach>
						
					</select> <input type="text" name="prodName" value="${condition.prodName}" />
