 ...
 * 2. Exception : 필요하다면 의도적으로 발생시킬 수 있고, 원하는 형태로 처리가 가능한 상황.
 * 		1) checked exception(Exception) : 처리하지 않으면, 컴파일 에러를 발생시키는 예외 ex) IOException, SQLException
 * 		2) unChecked exception(RuntimeException) : 직접 처리하지 않아도, 자동으로 최종 JVM에가 까지 전달(throw) 되는 예외.
 ...
 * 				(checked exception - unchechecked exception 전환)
